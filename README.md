...
- PHP 7.x/8.x
- MySQL / MariaDB
- HTML, CSS (pasta `style/`), assets estáticos
- Funciona via XAMPP/WAMP ou servidor LAMP
...
```bash
mysql -u seu_usuario -p nome_do_banco < backend/schema.sql
```

4. Configure as credenciais do banco em [backend/database.php](backend/database.php) (usuário, senha, nome do banco).
5. Acesse no navegador: `http://localhost/Sistema-de-Registro-e-Monitoramento-de-Ocorrencias-Disciplinares/` (ou caminho conforme sua instalação).
...
- [backend/edit_aluno.php](backend/edit_aluno.php) — edição de alunos
- [backend/deletar_aluno.php](backend/deletar_aluno.php) — remoção de alunos
...
- Crie um branch com um nome descritivo e envie um pull request com commits pequenos e claros.
- Teste as alterações localmente antes de submeter.

## Licença

Este projeto não possui licença especificada. Selecione e adicione uma licença (ex.: MIT) criando um arquivo `LICENSE` na raiz do repositório.

## Contato

Para dúvidas ou suporte, abra uma issue no repositório ou contate o mantenedor do projeto.
